...
def get_initial_data():
    ...
    attendees_cursor = conn.execute('SELECT al.id AS log_id, s.system_id, al.seat_number, al.entry_time, al.exit_time FROM attendance_logs al JOIN students s ON al.system_id = s.system_id WHERE al.entry_time >= ? ORDER BY al.entry_time ASC', (start_of_day_utc.isoformat(),))
    current_attendees = [{**dict(row), **all_students.get(row['system_id'])} for row in attendees_cursor.fetchall() if all_students.get(row['system_id'])]
    conn.close()
